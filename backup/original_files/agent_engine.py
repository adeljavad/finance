...
from .classification.ai_classifier import classify_financial_question_ai, is_data_related_query_ai
from .classification.financial_classifier import classify_financial_question, is_data_related_question
...
class AgentEngine:
    ...
    def _classify_query(self, query: str, context: str = "") -> str:
        """طبقه‌بندی سوال با استفاده از سیستم هوشمند"""
        ...
        # 3. استفاده از سیستم طبقه‌بندی هوشمند AI
        try:
            # ابتدا از سیستم AI استفاده کن
            ai_classification = classify_financial_question_ai(query)
            
            if ai_classification['needs_tool']:
                # سوال روی دیتای مالی است
                return 'data_analysis'
            
            # بررسی با سیستم ساده‌تر
            simple_classification = classify_financial_question(query)
            
            if simple_classification['category'] == 'data_related':
                return 'data_analysis'
            elif simple_classification['category'] == 'general_finance':
                return 'general_finance'
            elif simple_classification['category'] == 'greeting':
                return 'greeting'
            elif simple_classification['category'] == 'help':
                return 'help'
            
        except Exception as e:
            logger.warning(f"خطا در طبقه‌بندی هوشمند: {e}")
            # fallback به سیستم قدیمی
            if self._is_data_related_query(query):
                return 'data_analysis'
            elif self._is_financial_query(query):
                return 'general_finance'
    ...
    def _is_data_related_query(self, query: str) -> bool:
        """تشخیص سوالات مرتبط با داده (fallback)"""
        return is_data_related_question(query)
    
    def _is_financial_query(self, query: str) -> bool:
        """تشخیص سوالات مالی (fallback)"""
        from .classification.financial_classifier import is_general_finance_question
        return is_general_finance_question(query)
    ...
    def run(self, query: str, session_id: str = "default", user_id: str = None) -> str:
        ...
        try:
            ...
            if query_type == 'no_data':
                ...
                response = self._handle_general_finance_query(query)
            
            elif query_type == 'greeting':
                response = self._handle_greeting_query(query)
            
            elif query_type == 'help':
                response = self._handle_help_query(query)
    ...
    def _handle_general_finance_query(self, query: str) -> str:
        """پردازش سوالات عمومی مالی"""
        # استفاده از RAG برای پاسخ‌های تخصصی‌تر
        try:
            rag_results = self.rag.search(query, top_k=3)
            if rag_results and len(rag_results) > 0:
                # ترکیب نتایج RAG با LLM
                context = "\n".join([r['content'] for r in rag_results[:2]])
                prompt = f"""
                شما یک مشاور مالی متخصص هستید. بر اساس اطلاعات زیر به سوال کاربر پاسخ دهید:
                ...
                لطفاً پاسخ کامل و تخصصی ارائه دهید.
                """
            ...
            logger.warning(f"خطا در استفاده از RAG برای سوال مالی: {e}")
        
        # اگر RAG کار نکرد، از LLM مستقیم استفاده کن
        return self._ask_llm_directly(query)
    
    def _handle_greeting_query(self, query: str) -> str:
        """پردازش احوال‌پرسی"""
        from .classification.financial_classifier import get_financial_fallback_response
        return get_financial_fallback_response(query)
    
    def _handle_help_query(self, query: str) -> str:
        """پردازش درخواست راهنما"""
        from .classification.financial_classifier import get_financial_fallback_response
        return get_financial_fallback_response(query)
